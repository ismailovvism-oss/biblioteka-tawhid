...
| **1** | Клиент + UI входа (магик-линк); `mergeMyBooks` (публичные ∪ мои) | Аккаунты, «моя библиотека» | ✅ готово |
| **2** | Приватный путь чтения (`bookFileText` через подписанные URL) + `tools/upload-private.js` | **Приватное чтение онлайн** | 🔶 код читалки готов; нужен инструмент заливки + данные |
| **3** | Edge Function + вебхук платёжки → `entitlements`; платные книги | Подписки | ⏳ позже |

Фаза 1 реализована в `app.js` (`setupAccount`/`updateAcctUI`/`mergeMyBooks`) + `supabase/client.js`.
Приватный путь чтения (`bookFileText`, флаги `bookPrivate`/`bookStorageBase`) — тоже в `app.js`:
он **аддитивен и не активен, пока нет приватных книг**, поэтому публичная читалка не затронута.
Остаётся заливка первой приватной книги (фаза 2, `tools/upload-private.js` под `service_role`).
