...
2. Прислать в приложение (публично, не секрет): **Project URL** и **anon public key** —
   пропишем в конфиг клиента.
3. Прогнать SQL-миграцию (таблицы + RLS + бакет) — дам готовым файлом `supabase/schema.sql`.
4. `service_role`-ключ — НЕ присылать; понадобится только в фазе 3 и вводится в секреты
   Supabase, не в репозиторий.
...
## Открытые вопросы (к согласованию перед фазой 1)

1. **Вход:** пароль, магик-линк или оба? (Магик-линк проще для одного автора, без пароля.)
2. **Заливка приватных книг:** тем же импортёром в бакет через CLI, или мини-форма загрузки
   в приложении? (Для MVP — CLI/скрипт.)
3. **Регистрация:** открытая или только по приглашению/только владелец? (Пока книг мало —
   вероятно, только владелец.)
4. **Провайдер платежей** — решаем в фазе 3 (см. «География»).
