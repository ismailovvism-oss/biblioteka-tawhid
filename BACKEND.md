...
| Папка в формате Контракта | **`~/biblioteka-private/<id>/`** | нет | исходник для заливки + бэкап |
| То, что читает приложение | **бакет `book-content/<id>/`** | да | раздача по правам |

**В `books/` приватную книгу класть нельзя** — и это не про аккуратность: попав в коммит,
файл остаётся в истории git навсегда, а репозиторий публичный. Отсюда отдельная папка
`~/biblioteka-private/` вне репозитория — зеркало `books/` для непубличного.
...
двух копиях — мастер в облаке и папка `~/biblioteka-private/`. Пропадёт проект Supabase —
восстановишь заливкой, но только если цела вторая. То есть эта папка **не** временный
артефакт, в отличие от публичного пути.
